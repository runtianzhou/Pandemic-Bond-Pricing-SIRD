...
#    - DAILY storage: dt = 1 day (x-axis)
#    - INTERNAL simulation step: dt_n (sub-steps within each day)
# 2) Convert to daily new: ΔI(t), ΔD(t)
# 3) Compute 7-day moving averages: maI(t), maD(t)
# 4) Plot maI and maD on ONE graph with log y-axis + thresholds + crossings
############################################################
...
CFG <- list(
  ...
  dt_n   = 1,       # <- NEW: internal sub-step within each day (try 0.0001 if you want)
  M      = 1,           # M=1 for clarity
  I0     = 3,
  D0     = 1,
  KI     = 833e6,
  ...
  sigmaD = 0.1,
  rho    = 0.5,
  seed   = 123,        # set 42 for reproducible; NULL for random each run
...
#    - store DAILY (dt=1)
#    - simulate internally using dt_n (sub-steps within each day)
#
# Model:
...
simulate_alg1_ID <- function(cfg) {
  ...
  # ---- partition each day into an integer number of sub-steps ----
  # so the stored grid is exactly at integer days (0,1,2,...,N).
  n_sub  <- as.integer(round(dt / dt_n))
  if (n_sub < 1) n_sub <- 1
  dt_eff <- dt / n_sub               # effective internal step (exactly divides dt)
  
  # RNG seed
  set.seed(seed)
  
  # storage at integer days
  ...
  em_steps_performed <- 0L          # counts updates across all paths
  normals_generated  <- 0L          # counts scalar normals generated
  
  # ---- DAY LOOP (store end-of-day values) ----
  for (t_day in 2:(N + 1)) {
    
    # start-of-day state (yesterday's stored value)
    I_curr <- I[t_day - 1, ]
    D_curr <- D[t_day - 1, ]
    
    # ---- SUB-STEP LOOP (simulate within the day) ----
    for (k in 1:n_sub) {
      
      # independent increments for internal dt_eff
...
plot_maI_maD_one_graph_logy <- function(days, maI, maD, cfg) {
  ...
  # ---- y limits ----
  y_min_plot <- max(cfg$y_min, eps_y)
  ...
  # ---- thresholds ----
  abline(h = theta_I_plot, lwd = 3, lty = 4, col = col_I)
  abline(h = theta_D_plot, lwd = 3, lty = 4, col = col_D)
  
  text(x_min + 0.85 * (x_max - x_min), theta_I_plot,
       paste0("theta_I = ", format(theta_I, big.mark=",")),
       col = col_I, pos = 3)
  text(x_min + 0.85 * (x_max - x_min), theta_D_plot,
  ...
  if (is.finite(cross_I$first)) {
    ...
    text(cross_I$first, y_max_plot / 1.2, paste0(" I first = ", cross_I$first), col = col_first, pos = 4)
  }
  ...
  if (is.finite(cross_D$last)) {
    ...
    text(cross_D$last, y_max_plot / 5.8, paste0("D last = ", cross_D$last), col = col_last, pos = 4)
  }
  ...
  invisible(list(
    cross_I = cross_I, cross_D = cross_D,
